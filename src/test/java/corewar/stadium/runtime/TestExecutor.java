...
import corewar.stadium.StadiumShipForkBuilder;
...
public class TestExecutor {
	...
	@DataPoint
	public static final TP NOP = new TP(ship().get(), p().get(), 0, ship().get(), Executors::executeNop);

	@DataPoint
	public static final TP CRASH = new TP(ship().get(), p().get(), 0, ship().crashed().get(), Executors::executeCrash);
	...
	public static final TP CHECK_LAST_ZONE = new TP(
		ship()
			.pcw0((Constants.LAP_NUMBER * Constants.CHECKPOINTS_PER_LAP - 1) * Constants.CHECK_ZONE_SIZE + InstructionType.CHECK.getSize())
			.checked(1, 22)
			.get(),
		p().get(), 54_548,
		ship()
...
//	public static final TP WRITE = new TP();
	...
	private static StadiumShipForkBuilder ship() {
		return new StadiumShipForkBuilder(mainShip());
	}

	private static StadiumShip mainShip() {
		return new StadiumShip("", "", "", new Track());
	}
	...
	private static final class TP {
		...
		private final StadiumShipFork expected;
		private final Executors.Executor executor;

		private TP(StadiumShipFork ship, InstructionParameter param, int cycle, StadiumShipFork expected, Executors.Executor executor) {
			this.ship = ship;
			this.param = param;
			this.cycle = cycle;
